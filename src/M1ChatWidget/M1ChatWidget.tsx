...
import { M1ChatBody } from './M1ChatBody';

interface M1ChatWidgetProps {
  sdk: ChatSdk;
...
export const M1ChatWidget: FC<M1ChatWidgetProps> = ({ 
  sdk, 
  thread, 
  onBack,
  ...
  return (
    <div className={`chat-widget ${isMinimized ? 'minimized' : ''} ${isFullscreen ? 'fullscreen' : ''}`}>
      ...
      {/* Chat Body and Footer - Only show if not minimized */}
      {!isMinimized && thread && (
        <M1ChatBody sdk={sdk} thread={thread} />
      )}
    </div>
  );
};